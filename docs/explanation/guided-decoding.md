...
This is the load-bearing assumption behind the OpenAI-compatible adapter.
When the client injects the `tools` field into a request, it is implicitly
asserting two things about the upstream runtime:
