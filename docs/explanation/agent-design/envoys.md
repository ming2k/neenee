...
The `envoy` tool spawns an isolated child agent to investigate a sub-question
...
[`envoy`](../../reference/tools/envoy.md).
...
3. **Parallelizable investigation.** The model can dispatch several `envoy`
   calls to map different parts of a problem, then act on the synthesized
   findings.

## The `envoy` tool

The `envoy` tool is the one built-in tool whose result is not a single value
...
The envoy shares exactly one thing with the parent — the model provider —
and nothing else:
...
The filesystem is implicitly shared because the envoy inherits the process
working directory; its profile gates *what* it may write through the
`WriteScope` boundary (ADR-0028).

## Profiles

An envoy's behaviour is not hardcoded in the dispatch tool. It is the output
...
How a profile decides which of the parent's tools an envoy may actually use.
...
- **Recursion is impossible.** `envoy` marks itself `spawns_envoy()`, so
  it is excluded from every envoy regardless of profile. No name list is
...
name-driven and missed `ask_user` (which is `Read`), so the envoy could call
...
An envoy is **not** fire-and-forget. Communication is full-duplex
...
- **Up.** The child's `PermissionRequest` and `UserQuestionRequest` events are
  forwarded as `EnvoyEvent`s to the parent harness (and through it to the
  TUI), instead of being dropped. The child's own broker / `ask_user` oneshot
  stays parked until a reply arrives.
- **Down.** Each child installs a steering inbox and exposes an `EnvoyHandle`
  (`Weak<Agent>` + a sender). The dispatch tool keeps an `EnvoyRegistry`
...
How an envoy executes once dispatched: the event stream it emits, how the TUI
renders it, and how it fails.

### Event streaming

The envoy is a real agent, so it emits the full event stream a parent does.
Each child event is wrapped and forwarded to the parent, so the TUI builds the
nested view in real time:

```text
envoy event ──forward──► wrapped child event
                                │
...
The `envoy` step renders inline as one summary line plus a live status line.
...
- The entire footer — activity bar, input box, hint bar — is
  hidden. The envoy view is read-only chrome.
...
An envoy that hits a harness safety bound (a read-only stall, repeated
