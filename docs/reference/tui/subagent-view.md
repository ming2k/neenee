...
The live status line comes from `TranscriptMessage::subagent_status_line`
...
`self.messages`. The footer (status bar, plan panel, input box, hint bar)
...
| File | Responsibility |
|------|----------------|
| `render/step/renderers.rs` | `draw_subagent_inline_step`, `draw_subagent_bar` |
