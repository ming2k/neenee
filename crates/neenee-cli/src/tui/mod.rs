...
fn extract_selection_text(
    ...
    let on_input = match sel {
        ...
        SelectionState::Block { message_idx, .. } => *message_idx == crate::tui::render::INPUT_MSG_IDX,
...
fn spawn_clipboard_paste(tx: &mpsc::UnboundedSender<clipboard::ClipboardRead>) {
    ...
    tokio::spawn(async move {
        let read =
            match tokio::time::timeout(std::time::Duration::from_secs(3), crate::tui::clipboard::read())
                .await
            {
                Ok(inner) => inner,
...
fn apply_clipboard_paste(app: &mut App, read: clipboard::ClipboardRead) {
    ...
    match read {
        clipboard::ClipboardRead::Image { data, mime } => {
            let encoded = crate::tui::clipboard::base64_image(&data);
