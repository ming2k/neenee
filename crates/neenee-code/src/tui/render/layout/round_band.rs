...
//! message, a single-line header:
//!
//! ```text
//! round 2 · sonnet · 3 calls
//! ```
//!
//! rendered in the brand accent on a subtly-raised background band, spanning
//! the inset transcript width. The band's background continues (one row) to
//! visually "open" the group; the group closes implicitly at the next group's
//! header or the stream end (no hard closing border — see design notes). This
//! keeps the layout cheap (no per-cell background fill across the group's
//! body, which would require repaint coordination with every drawer) while
//! giving each round a clear, labelled anchor.
...
/// Paint the round header row: `round N · model · K calls` on a raised band.
fn draw_round_header(
    ...
    // Two-tone label: `round N` is the brand-accent anchor, the rest (model,
    // call count) reads as muted metadata on the same line.
    let bg = theme.raised();
    let accent = Style::default()
        .bg(bg)
        .fg(theme.brand())
        .add_modifier(Modifier::BOLD);
    let meta = Style::default().bg(bg).fg(theme.muted());
    ...
    let mut used: usize = 1 + "round ".len() + round.to_string().len();
    let model_name = msg
        .model
        .as_deref()
        .filter(|m| !m.is_empty())
        .map(crate::tui::model_display_name);
    if let Some(name) = &model_name {
        let seg = format!(" · {}", name);
        used += seg.chars().count();
        spans.push(Span::styled(seg, meta));
    }
    let calls_seg = format!(" · {} {}", calls, if calls == 1 { "call" } else { "calls" });
    used += calls_seg.chars().count();
    spans.push(Span::styled(calls_seg, meta));

    let pad = (band.width as usize).saturating_sub(used);
