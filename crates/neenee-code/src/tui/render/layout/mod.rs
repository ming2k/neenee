...
//! - [`round_band::RoundBand`] — option C: each tool round is grouped into a
//!   labelled band with a header row (`round N · model · K calls`).
//!
...
use super::{HeightCache, MESSAGE_GAP_ROWS};
...
pub struct Stream<'a, 'f> {
    ...
    /// `(provider, model)` of the last attribution badge drawn, so the badge
    /// shows once per model-run rather than per message.
    pub last_shown_attribution: Option<(String, String)>,
}

impl<'a, 'f> Stream<'a, 'f> {
    /// Draw the model attribution badge above an assistant-side message, if
    /// the message carries a fresh attribution. Idempotent w.r.t. the
    /// `last_shown_attribution` dedup — layouts can call it unconditionally at
    /// the top of each message and it will no-op when the model hasn't changed.
    pub fn badge(&mut self, mi: usize) {
        ...
        if is_assistant_side
            ...
            && self.last_shown_attribution.as_ref() != Some(&attribution)
        {
            super::draw_attribution_badge(
                self.frame,
                self.band,
                &attribution,
                &mut self.skip_rows,
            ...
            self.last_shown_attribution = Some(attribution);
        }
    }
