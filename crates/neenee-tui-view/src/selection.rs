...
use crate::layout::{LayoutMap, SemanticCursor, TableCellSegment};
use std::borrow::Cow;

/// Re-exported grapheme-boundary primitives so the rest of the crate keeps the
/// existing `crate::selection::…` import path. The implementations live in
/// the engine ([`neenee_tui::text`]), the single owner of grapheme/width
/// measurement.
pub use neenee_tui::text::{floor_grapheme_boundary, inclusive_grapheme_end};
